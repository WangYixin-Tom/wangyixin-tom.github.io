...
**秒杀活动可以分为3个阶段：秒杀前：**用户不断刷新商品详情页，页面请求达到瞬时峰值。**秒杀开始：**用户点击秒杀按钮，下单请求达到瞬时峰值。**秒杀后：**一部分成功下单的用户不断刷新订单或者产生退单操作，大部分用户继续刷新商品详情页等待退单机会。消费者提交订单，一般做法是利用数据库的行级锁，只有抢到锁的请求可以进行库存查询和下单操作。但是在高并发的情况下，数据库无法承担如此大的请求，往往会使整个服务blocked，在消费者看来就是服务器宕机。
