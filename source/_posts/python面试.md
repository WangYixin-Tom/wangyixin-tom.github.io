...
- True 和 False 在 Python2 中是全局变量，分别对应 1 和 0，可以指向其它对象。 Python3  True 和 False 变为两个关键字，永远指向两个固定的对象，不允许再被重新赋值。
- Python 2中print是特殊语句，Python 3中print则变成了函数，需要加上括号。
- 在Python 2中，3/2是整数，在Python 3中，浮点数。
- python2 range返回列表，python3 range中返回迭代器，节约内存。
...
可变数据类型：当该数据类型对应的变量的值发生了变化时，如果它对应的内存地址不发生改变，那么这个数据类型就是 可变数据类型。也叫引用类型

不可变数据类型：当该数据类型对应的变量的值发生了变化时，如果它对应的内存地址发生了改变，那么这个数据类型就是 不可变数据类型。也叫值类型。
...
带参数时，返回参数的属性、方法列表。如果参数包含方法`__dir__()`，该方法将被调用。如果参数不包含`__dir__()`，该方法将最大限度地收集参数信息。
...
`__new__`才是实例化对象调用的第一个方法，用来创造一个类的实例的，取下cls参数，把其他参数传给`__init__`.
...
**迭代器协议**：要求一个 `__iter__()` 方法返回一个特殊的迭代器对象， 这个迭代器对象实现了 `__next__()` 方法并通过 `StopIteration` 异常，标识迭代的完成。

**迭代器对象**：实现了迭代器协议的对象/被`next()`函数调用并不断返回下一个值的对象称为迭代器。

**使用**

在类中实现两个方法 `__iter__()`与 `__next__() `。
...
add函数原本接收两个参数a和b，经过partial包装之后，a参数的值被固定为了1，新的add对象（注意此处add已经是一个可调用对象）只需要接收一个参数即可。
...
第一个 x和第二个 x 是两个不同 property实例。但他们都属于同一个描述符类（property），当赋值时，就会进入 `property.__set__`，当对math 进行取值里，就会进入 `property.__get__`。
...
for循环就是基于迭代器协议提供了一个遍历所有对象的方法，

**通过调用`iter()`方法执行（字符串，元组，字典，集合，文件）对象内部的`__iter__`方法，获取一个迭代器，然后使用迭代器协议去实现循环访问，这样所有的对象就都可以通过for循环来遍历了，**当元素循环完时，会触发StopIteration异常，for循环会捕捉到这种异常，终止迭代

### 深拷贝和浅拷贝

浅拷贝：在另一块地址中创建一个新的变量或容器，但是容器内的元素的地址均是源对象的元素的地址的拷贝。也就是说新的容器中指向了旧的元素（ 新瓶装旧酒 ）。
...
复制对象中包含的非可变数据类型：开辟新的内存，有新的引用。原来值的改变并不会影响浅复制的值。
...
1. 一种更加优雅的方式，操作（创建/获取/释放）资源，如文件操作、数据库连接；处理异常；
...
- ascii  最多只能用8位来表示一个字符，最多只能表示 256 个符号。
- unicode： 所有字符（无论英文、中文等） 需要2个字节表示
...
模块 pickle 实现了对一个 Python 对象结构的二进制序列化和反序列化。 "pickling" 是将 Python 对象及转化为一个字节流的过程，而 "unpickling" 是相反的操作,会将字节流转化回一个对象层次结构。
...
只有属性没有方法的类，用于组织数据，称为数据类或，就像C语言中的结构体。在Python中可以用`namedtuple`（命名元组）来替代这种类。
...
命名元组与普通元组一样是不可变容器，一旦将数据存储在`namedtuple`的顶层属性中，数据就不能再修改了，对象上的所有属性都遵循“一次写入，多次读取”的原则。
...
**缺点：**经典类的查找顺序从左到右深度优先的规则，在访问`d.foo1()`的时候,D->B->A,找到了`foo1()`,调用A的`foo1()`，导致C重写的`foo1()`被绕过
...
- 静态方法: 是一个任意函数，在其上方使用 @staticmethod 进行装饰
  ...
  >
  > 使用实例对象调用的类方法或静态方法，最终都会转而通过类对象调用。
...
接口类中定义了一些接口，子类继承接口类，并且实现接口中的功能
...
抽象类规定子类应该具备的功能。 Python 原生仅支持抽象类，不支持接口类，abc模块就是用来实现抽象类的。
...
**元类是用来创建类的类。**metaclass允许你创建类或者修改类。类是metaclass创建出来的“实例”。
...
[ORM]: https://www.liaoxuefeng.com/wiki/1016959663602400/1017592449371072	"ORM"
...
这是因为采用开放寻址法中，**遇到哈希冲突时会找到下一个合适的位置，**例如ABC构成了探测链，查找元素时如果hash值相同，那么也是**顺着这条探测链不断往后找**，当删除探测链中的某个元素时，如果直接把B从哈希表中移除，即变成Unused状态，那么C就不可能再找到了，因此需要Dummy保证探测链的连续性。
...
出于性能考虑，对于小整数，Python使用**小整数对象池**small_ints缓存了[-5，257）之间的整数，该范围内的整数在Python系统中是共享的，值相同就属于同一个对象。

而超过该范围的整数，值相同但对象不一定是同一个。

CPython底层为了进一步提升性能还做了另一个设定，对于同一个代码块中值不在`small_ints`缓存范围内的整数，如果同一个代码块中已经存在一个值与其相同的整数对象，那么就直接引用该对象，否则创建新的`int`对象。
...
**原理：**每个对象维护一个ob_ref字段，用来记录该对象当前被引用的次数，每当新的引用指向该对象时，它的引用计数加1，每当该对象的引用失效时计数减1，一旦对象的引用计数为0，该对象立即被回收，对象占用的内存空间将被释放。
...
在执行标记-清除算法时减小遍历的对象数，从而提高垃圾回收的速度。

给对象定义了三种世代，每一个新生对象在generation 0中，如果它在一轮gc扫描中活了下来，那么它将被移至generation 1,在那里他将较少的被扫描，如果它又活过了一轮gc,它又将被移至generation 2，在那里它被扫描的次数将会更少。
...
协程是一种用户态的轻量级线程，协程的调度完全由用户控制。协程拥有自己的寄存器上下文和栈。协程调度时，将寄存器上下文和栈保存到其他地方，在切回来的时候，恢复先前保存的寄存器上下文和栈，基本没有内核切换的开销，可以不加锁的访问全局变量，上下文的切换非常快。
